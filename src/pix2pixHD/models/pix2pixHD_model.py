...
class GANcing(Pix2PixHDModel):
    ...
    def initialize(self, opt):
        ...
        if self.use_features:
            netG_input_nc += opt.feat_num      
        # Add the channels for the previous image and the label    
        netG_input_nc += 3            
        self.netG = networks.define_G(netG_input_nc, opt.output_nc, opt.ngf, opt.netG, 
                                      opt.n_downsample_global, opt.n_blocks_global, opt.n_local_enhancers, 
                                      opt.n_blocks_local, opt.norm, gpu_ids=self.gpu_ids)        
    ...
    def forward(self, label, inst, image, feat, previous_label, previous_img, infer=False):
        # Encode Inputs
        input_label, inst_map, real_image, feat_map, previous_real_img, previous_label_input = self.encode_input(label, inst, image, feat, previous_img, previous_label)  
        ...
        if self.use_features:
            if not self.opt.load_features:
                feat_map = self.netE.forward(previous_fake_image, inst_map)
            input_concat = torch.cat((input_label, feat_map), dim=1)
        else:
            input_concat = input_label
        ...
        loss_G_VGG = 0
        if not self.opt.no_vgg_loss:
            print(self.criterionVGG(fake_image, real_image))
            print(self.criterionVGG(previous_fake_image, previous_img.cuda()))
            loss_G_VGG = (self.criterionVGG(fake_image, real_image) + self.criterionVGG(previous_fake_image, previous_img.cuda()))  * self.opt.lambda_feat

        # Only return the fake_B image if necessary to save BW
        return [self.loss_filter(loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_D_real, loss_D_fake), None if not infer else fake_image]

    def inference(self, label, inst, previous_img):
        # Encode Inputs
        input_label, inst_map, previous_img, _, _, _ = self.encode_input(Variable(label), Variable(inst), previous_img.detach(), infer=True)
        ...
        input_concat = torch.cat((input_concat, previous_img), dim=1)
        if torch.__version__.startswith('0.4'):
            with torch.no_grad():
                fake_image = self.netG.forward(input_concat)
        else:
...
class InferenceModelGANcing(GANcing):
    def forward(self, inp):
        label, inst, previous_img = inp
        return self.inference(label, inst, previous_img)
